...
#[doc = " Represents a single database partition — its in-memory columnar store, persistent"]
#[doc = " storage, write-ahead log, and the command channel that drives it."]
#[doc = ""]
#[doc = " Each partition runs on its own native OS thread. The [`run()`](Partition::run) method"]
#[doc = " is the thread's main loop: it drains incoming [`PartitionCommand`]s, executes batched"]
#[doc = " writes (Group Commit), and performs QSBR maintenance to safely reclaim old RCU"]
#[doc = " snapshots."]
#[repr(C, align(64))]
pub struct Partition<const N: usize> {
    #[doc = " Backing columns array."]
    pub columns: Arc<AtomicPtr<Columns<N>>>,
    #[doc = " Thread-safe pointers to vector snapshots."]
    pub shared_pointers: Arc<AtomicPtr<AHashMap<usize, MultiVectorPointer>>>,
    #[doc = " Receiver channel for incoming commands."]
    pub writer_rx: alloc::boxed::Box<dyn crate::io::platform::MessageQueue>,
    #[doc = " Quiescent State Based Reclamation (QSBR) manager."]
    ...
    #[doc = " Write-Ahead Log provider."]
    pub wal: Arc<dyn WalProvider>,
    #[doc = " Count of items in the Bloom filter."]
    pub bloom_count: usize,
    #[doc = " Total number of bits in the Bloom filter."]
    pub bloom_bits: usize,
    #[doc = " File system abstraction."]
    pub fs: Arc<dyn FileSystem>,
}
impl<const N: usize> Partition<N> {
    #[doc = " Creates a new `Partition` worker instance."]
    #[doc = ""]
    #[doc = " # Arguments"]
    #[doc = ""]
    #[doc = " * `writer_rx` — The command channel this partition will drain in its event loop."]
    #[doc = " * `columns`   — Shared atomic pointer to the columnar data store."]
    #[doc = " * `wal`       — Write-Ahead Log provider used to persist commands before applying them."]
    #[doc = " * `workers`   — QSBR worker list shared across all partition threads."]
    #[doc = " * `storage_path` — File-system path for the on-disk storage of this partition."]
    #[doc = " * `fs`        — File-system abstraction (allows injection of in-memory FS for tests)."]
    #[doc = " * `shared_pointers` — Atomic pointer to the global entity-pointer map (RCU-managed)."]
    #[doc = " * `bloom_filter`    — Atomic pointer to the Bloom filter for fast existence checks."]
    #[doc = " * `hot_index`       — Dual-cache heat tracker shared with the query path."]
    #[doc = " * `partition_id`    — Numeric identifier of this partition within the database."]
    ...
    #[doc = " Inserts `entity_id` into the Bloom filter and triggers a rebuild when the filter"]
    #[doc = " exceeds 70 % occupancy."]
    ...
    #[doc = " Doubles the Bloom filter's bit-count and repopulates it from the current disk index,"]
    #[doc = " then atomically swaps in the new filter via RCU."]
    ...
    #[doc = " Runs the partition's main event loop on the calling thread (intended to be the"]
    #[doc = " partition's dedicated OS thread)."]
    #[doc = ""]
    #[doc = " The loop:"]
    #[doc = " 1. Blocks on the command channel, waiting for the next [`PartitionCommand`]."]
    #[doc = " 2. Drains up to 1 000 additional pending commands without blocking (Group Commit"]
    #[doc = "    batching — amortises WAL and flush overhead across many writes)."]
    #[doc = " 3. Appends all [`WriteCommand`]s in the batch to the WAL before applying them."]
    #[doc = " 4. Applies the entire batch via [`apply_batch_commands`](Self::apply_batch_commands)."]
    #[doc = " 5. Flushes the storage layer."]
    #[doc = " 6. Runs one round of QSBR maintenance to reclaim deferred RCU garbage."]
    #[doc = ""]
    #[doc = " The loop exits when a [`PartitionCommand::Shutdown`] is received or the channel"]
    #[doc = " is closed."]
    #[doc = ""]
    #[doc = " Under `std`, a background compaction thread is also spawned that triggers"]
    #[doc = " storage compaction every 300 seconds."]
    pub fn run(mut self) {
        #[cfg(feature = "std")]
    ...
    #[doc = " Applies a batch of [`PartitionCommand`]s to the in-memory columnar store and"]
    #[doc = " atomically publishes the updated [`MultiVectorPointer`] map via RCU swap."]
    ...
    #[doc = " Inserts or updates a single entity's attributes into the columnar store, updates"]
    #[doc = " the Bloom filter and hot-index, and persists the entity to storage."]
    fn process_insert(
        &mut self,
    ...
    #[doc = " Promotes a cold entity loaded from disk into the in-memory columnar store,"]
    #[doc = " returning its new [`MultiVectorPointer`]."]
    fn process_promote(
        &mut self,
    ...
    #[doc = " Convenience wrapper — applies a single [`PartitionCommand`] as a one-element batch."]
    fn apply_command(&mut self, cmd: PartitionCommand) {
        self.apply_batch_commands(vec![cmd]);
    }
    #[doc = " Appends a single [`WriteCommand`] to the WAL (kept for direct-call use-cases;"]
    #[doc = " the hot path uses [`WalProvider::append_batch`] instead)."]
    #[allow(dead_code)]
    fn log_wal(&mut self, cmd: &WriteCommand) {
        let _ = self.wal.append(cmd);
    }
    #[doc = " Replays all WAL entries on startup to restore the in-memory columnar state after"]
    #[doc = " a crash or restart."]
    #[doc = ""]
    #[doc = " Reads the entire WAL byte stream, decodes each length-prefixed [`WriteCommand`]"]
    #[doc = " record in order, and re-applies them through [`apply_command`](Self::apply_command)."]
    #[doc = " Truncated or corrupt trailing records are silently skipped."]
    #[doc = ""]
    #[doc = " # Errors"]
    #[doc = ""]
    #[doc = " If [`WalProvider::read_all`] returns an error the method returns early without"]
    #[doc = " applying any commands."]
    ...
    #[doc = " Returns the existing string [`ColumnArray`](crate::core::column::ColumnArray) for `name`,"]
    #[doc = " or creates and atomically publishes a new one via RCU swap."]
    ...
    #[doc = " Returns the existing integer [`ColumnArray`](crate::core::column::ColumnArray) for `name`,"]
    #[doc = " or creates and atomically publishes a new one via RCU swap."]
    ...
    #[doc = " Returns the existing blob [`ColumnArray`](crate::core::column::ColumnArray) for `name`,"]
    #[doc = " or creates and atomically publishes a new one via RCU swap."]
